...
export default function Navbar() {
  ...
  return (
    <header className="text-gray-50 fixed top-0 left-0 right-0 w-full z-50 bg-hoverColor/30 backdrop-filter backdrop-blur-3xl mx-auto flex flex-col overflow-hidden px-2 py-1 lg:flex-row lg:items-center rounded-full border border-white my-4 max-w-7xl">
      ...
      <nav aria-label="Header Navigation" className="p-2 peer-checked:pt-8 peer-checked:max-h-60 flex max-h-0 w-full flex-col items-center overflow-hidden transition-all lg:ml-24 lg:max-h-full lg:flex-row bg-transparent">
        ...
        <li className="lg:mr-4 bg-transparent">
            <Link
              ref={homeSectionRef}
              tabIndex={0} 
              className="rounded-xl px-4 py-2 text-primary transition hover:bg-hoverColor focus:bg-hoverColor focus:ring-1 focus:ring-primary  font-bold text-sm"
              to="/" onClick={(e) => handleLinkClick(e, "home")}
            >
              Home
            </Link>
          </li>
